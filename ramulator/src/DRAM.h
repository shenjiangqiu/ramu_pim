...
using namespace std;
template <typename T>
concept DramSpec = requires(T t) {
  typename std::integral_constant<int, (int)T::Level::MAX>;
  typename std::integral_constant<int, (int)T::Level::Row>;
  typename std::integral_constant<int, (int)T::Level::Bank>;
  typename T::State;
  typename std::integral_constant<int, (int)T::Command::MAX>;
};

namespace ramulator {

template <DramSpec T> class DRAM {
public:
...
private:
  ...
  deque<long> prev[int(
      T::Command::MAX)]; // the most recent history of when commands were issued
  ...
  function<typename T::Command(DRAM<T> *, typename T::Command cmd, int)>
      *prereq;
...
// register statistics
template <DramSpec T> void DRAM<T>::regStats(const std::string &identifier) {
  ...
  busy_cycles.name("busy_cycles" + identifier + "_" + to_string(id))
      .desc("(All-bank refresh only. busy cycles only include refresh time in "
            "rank level) The sum of cycles that the DRAM part is active or "
            "under refresh for level " +
            identifier + "_" + to_string(id))
      .precision(0);
  active_refresh_overlap_cycles
      .name("active_refresh_overlap_cycles" + identifier + "_" + to_string(id))
      .desc("(All-bank refresh only, only valid for rank level) The sum of "
            "cycles that are both active and under refresh per memory cycle "
            "for level " +
            identifier + "_" + to_string(id))
      .precision(0)
      .flags(Stats::nozero);
  ...
  average_serving_requests
      ...
      .desc("The average of read and write requests that are served in this "
            "DRAM element per memory cycle for level " +
            identifier + "_" + to_string(id))
      .precision(6);
...
template <DramSpec T> void DRAM<T>::finish(long dram_cycles) {
...
template <DramSpec T> DRAM<T>::~DRAM() {
  for (auto child : children)
    delete child;
}

// Insert
template <DramSpec T> void DRAM<T>::insert(DRAM<T> *child) {
  child->parent = this;
...
typename T::Command DRAM<T>::decode(typename T::Command cmd, const int *addr) {
  rdebug_with_target("DRAM_LEVEL", T::level_str[(int)level]);
  rdebug_with_target("DRAM_DECODING", spec->command_name[(int)cmd]);
  int child_id = addr[int(level) + 1];
  if (prereq[int(cmd)]) {
    typename T::Command prereq_cmd = prereq[int(cmd)](this, cmd, child_id);
    if (prereq_cmd != T::Command::MAX) {

      rdebug_with_target("DRAM_PRE", spec->command_name[(int)prereq_cmd]);
  ...
  if (child_id < 0 || !children.size()) {
    rdebug_with_target("DRAM_nochild", "return");
...
bool DRAM<T>::check_row_hit(typename T::Command cmd, const int *addr) {
  ...
  if (rowhit[int(cmd)]) {
    return rowhit[int(cmd)](
        this, cmd,
        child_id); // stop recursion: there is a row hit at this level
  }
...
void DRAM<T>::update_serving_requests(const int *addr, int delta, long clk) {
  ...
  if (delta == 1 && cur_serving_requests == 1) {
    ...
    for (const auto &ref : refresh_intervals) {
      active_refresh_overlap_cycles +=
          min(end_of_serving, ref.second) - ref.first;
    }
