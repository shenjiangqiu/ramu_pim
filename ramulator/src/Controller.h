...
public:
  ...
  void tick() {
    ...
    if (pending.size()) {
      ...
      if (req.depart <= clk) {
        rdebug(fmt::format("some req finished at cycle:{}", clk));
        if (req.depart - req.arrive > 1) { // this request really
                                           // accessed a row
    ...
    if (is_valid_req) {
      rdebug(fmt::format("find a actq request, addr:{}", req->addr));

      cmd = get_first_cmd(req);
      rdebug(fmt::format("cmd:{}", channel->spec->command_name[(int)cmd]));
      is_valid_req = is_ready(cmd, req->addr_vec);
      rdebug(fmt::format("is_valid_req:{}", is_valid_req));
    }
    ...
    if (!is_valid_req) {
      ...
      rdebug(fmt::format("find rd/wr req:{}", is_valid_req));
      if (is_valid_req) {
        cmd = get_first_cmd(req);
        is_valid_req = is_ready(cmd, req->addr_vec);
        rdebug(fmt::format("is_valid_req:{},cmd:{},addr: {}", is_valid_req,
                           channel->spec->command_name[(int)cmd], req->addr));
      }
    }
    ...
    if (!is_valid_req) {
      ...
      rdebug(fmt::format("no valid request found,issue victim size: {}",
                         victim.size()));
    ...
    if (req->is_first_command) {
      rdebug(
          fmt::format("first cmd:{}", channel->spec->command_name[(int)cmd]));
    ...
    rdebug(fmt::format("issue cmd:{}", channel->spec->command_name[(int)cmd]));
    issue_cmd(cmd, get_addr_vec(cmd, req));

    // check whether this is the last command (which finishes the request)
    // if (cmd != channel->spec->translate[int(req->type)]){
    if (cmd != channel->spec->translate[int(req->type)]) {
      rdebug(fmt::format("cmd:{} is not the translate one!",
                         channel->spec->command_name[(int)cmd]));
      if (channel->spec->is_opening(cmd)) {
        rdebug(fmt::format("cmd:{} is opening!",
                           channel->spec->command_name[(int)cmd]));
...
private:
  ...
  void issue_cmd(typename T::Command cmd, const vector<int> &addr_vec) {
    ...
    if (with_drampower) { // Yue: add drampower
      ...
      drampower->doCommand(DRAMPower::MemCommand::getTypeFromName(cmd_name),
                           bank_id, clk);
